...
CREATE TABLE sync_items (id INTEGER PRIMARY KEY, sync_target INT NOT NULL, sync_time INT NOT NULL DEFAULT 0, item_type INT NOT NULL, item_id TEXT NOT NULL, sync_disabled INT NOT NULL DEFAULT "0", sync_disabled_reason TEXT NOT NULL DEFAULT "", force_sync INT NOT NULL DEFAULT 0, item_location INT NOT NULL DEFAULT 1, sync_warning_ignored INT NOT NULL DEFAULT "0");
...
CREATE TABLE IF NOT EXISTS 'notes_fts_segments'(blockid INTEGER PRIMARY KEY, block BLOB);
CREATE TABLE IF NOT EXISTS 'notes_fts_segdir'(level INTEGER,idx INTEGER,start_block INTEGER,leaves_end_block INTEGER,end_block INTEGER,root BLOB,PRIMARY KEY(level, idx));
CREATE TABLE IF NOT EXISTS 'notes_fts_docsize'(docid INTEGER PRIMARY KEY, size BLOB);
CREATE TABLE IF NOT EXISTS 'notes_fts_stat'(id INTEGER PRIMARY KEY, value BLOB);
...
/* items_fts(id,title,body,item_id,item_type,user_updated_time,reserved1,reserved2,reserved3,reserved4,reserved5,reserved6) */;
CREATE TABLE IF NOT EXISTS 'items_fts_segments'(blockid INTEGER PRIMARY KEY, block BLOB);
CREATE TABLE IF NOT EXISTS 'items_fts_segdir'(level INTEGER,idx INTEGER,start_block INTEGER,leaves_end_block INTEGER,end_block INTEGER,root BLOB,PRIMARY KEY(level, idx));
CREATE TABLE IF NOT EXISTS 'items_fts_docsize'(docid INTEGER PRIMARY KEY, size BLOB);
CREATE TABLE IF NOT EXISTS 'items_fts_stat'(id INTEGER PRIMARY KEY, value BLOB);
